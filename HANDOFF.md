...
喊"小宇"能唤醒，说完话它能答上来，延迟体感可以接受。
**"反应慢"这件事也做完了**：TTS 换成本地 sherpa-onnx + DeepSeek 连接预热，
首句出声从 3~13 秒降到 **0.6 秒**。

**下一件要干的是 S4 记忆。** 性格（`config/persona.md`）已经有了，
但"记忆"现在是个空壳 —— **程序一关，它就完全不记得你**。原因和实测证据见 §6.1。
...
## 6.1 明天（2026-09-18）要做的：S4 记忆

**一句话**：现在的"记忆"是假的。程序一关，它就把你忘得干干净净。

### 🔴 P0 · 让它重启之后还记得你（预计 30~40 分钟，改动很小）
...
### 🟠 P1 · 让它自动记住"关于主人的事实"（预计 60 分钟）

**为什么**：`facts` 表建好了、`add_fact()` 写好了、
`recall()` 也接好了 —— 但 **`add_fact()` 一个调用方都没有**。
所以 `facts` 表永远是空的，`recall()` 永远返回空列表。
这是一条**从头到尾健全、就是没接上电源**的电路。

**怎么做**：每 `MemoryConfig.summarize_every`（现在写的是 20）轮，
拿最近的对话去问一次 DeepSeek："从这段对话里挑出**以后还用得上**的关于主人的事实"，
要求它输出 JSON 数组，然后 `add_fact()` 逐条存。

**两个必须处理的细节**：

- **去重**。同一件事说两遍不能存两条，否则 `recall()` 返回三条一样的。
  参考 `MemoryWebAssistant` 的"相似度 > 0.8 就不重复存"，
  但我们还没有向量 —— 先用简单办法：存之前拿新内容和已有 facts 做归一化字符串比对，
  或者干脆把"已有的 facts"一起塞给模型，让它自己别重复。
- **别让它记流水账**。"今天聊了天气"不是事实，"主人在准备考研"才是。
  提示词里要写清楚这个区别，并且限定条数（一次最多 3 条）。
...
- **S5 表情脸**：Live2D，旧手机浏览器当屏幕，接状态机的 `on_change`。
- **S6 眼睛 / 独立成体**：RapidOCR 读字 + InsightFace 认人；搬 N100。
- **虚拟环境**：用户之前选了"以后弄"。
